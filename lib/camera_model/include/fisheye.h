...
class FisheyeModel : public CameraModel {
 public:
  FisheyeModel() = default;
  FisheyeModel(const cv::Mat& camera_matrix, const cv::Mat& distort_coe,
  ...
  void UndistortPoints(const std::vector<cv::Point2d>& src,
                       std::vector<cv::Point2d>& dst) override;
  ...
  void SolvePnP(const std::vector<cv::Point3d>& point_3d,
                ...
                std::vector<double>& rvec, std::vector<double>& tvec) override;
